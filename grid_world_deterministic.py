    ...
    def __init__(self):
        ...
        for i in range(BOARD_ROWS):
            for j in range(BOARD_COLS):
                self.state_values[(i,j)] = 0 # set all initial values to 0
    ...
    def play(self, rounds=10):
        ...
        while i < rounds:
            ...
            if self.State.isEnd:
                ...
                self.state_values[self.State.state] = reward #optional
                print("Game End Reward", reward)

                print("state_values", self.state_values)
                for s in reversed(self.states):
                    print(f"Reward = {self.state_values[s]} + {self.lr}*({reward} - {self.state_values[s]})")
